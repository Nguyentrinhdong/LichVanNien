...
import com.bhmedia.lichvannien2016.ui.database.VanTrinhThangDataBaseHelper;
 ...
 */
@SuppressLint("NewApi")
public class KetQuaVanTrinhThangActivity extends FragmentActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tvTitle.setText("Vận trình tháng");
		VanTrinhThangDataBaseHelper vanTrinhNamDataBaseHelper = new VanTrinhThangDataBaseHelper(
				this);
		Bundle mBundle = getIntent().getExtras().getBundle("Bundle");
		tv_ngaySinhDuongLich
				.setText(mBundle.getString("NGAY") + " - "
						+ mBundle.getString("THANG") + " - "
						+ mBundle.getString("NAM"));

		String[] ThangAm = new String[] { "binh dan", "dinh mao", "mau thin",
				"ky ty", "canh ngo", "tan mui", "nham than", "quy dau",
				"giap tuat", "at hoi", "binh ty", "dinh suu" };
		String[] TuoiAm = new String[] { "ty", "suu", "dan", "mao", "thin",
				"ti", "ngo", "mui", "than", "dau", "tuat", "hoi" };

		Calendar calendar = Calendar.getInstance();
		CalendarUtil calendarUtil = new CalendarUtil();
		int[] convert = calendarUtil.convertSolar2Lunar(
				Integer.parseInt(mBundle.getString("NGAY")),
		...
		int pos_thang_am_lich = mBundle.getInt("THANG_AM");
		String thang_am_lich = ThangAm[pos_thang_am_lich];
		int pos_tuoiAm = (convert[2] + 8) % 12;
		String tuoiAm = TuoiAm[pos_tuoiAm];

		tv_ngaySinhAmLich.setText(calendar.get(Calendar.DAY_OF_MONTH) + " - "
				+ (calendar.get(Calendar.MONTH) + 1) + " - "
				+ calendar.get(Calendar.YEAR) + "");

		String nam_xem = mBundle.getString("NAM_XEM");

		if (mBundle != null) {

			VanTrinhNam listVanTrinhNam = vanTrinhNamDataBaseHelper
					.showVTThang(this, thang_am_lich, tuoiAm, nam_xem);

			if (listVanTrinhNam == null) {
				AlertDialog alertDialog = new AlertDialog.Builder(
						KetQuaVanTrinhThangActivity.this)
						.setCancelable(false)
						.setMessage(
								"Nội dung của tháng bạn cần xem chưa có, bạn vui lòng chọn lại tháng khác.")
			...
			html = "<html><body><font color ='#000000'" + str
					+ "</font></body></html>";
			...
			wv_ketqua_vantrinhnam.loadDataWithBaseURL(null, html, "text/html",
					"utf-8", null);
		...
		case R.id.iv_share:
			// -----------------------Send có lựa chọn------------------//
						...
						String shareBody = "THÔNG TIN GIA CHỦ CUNG CẤP:\n Ngày sinh dương lịch:"
								+ tv_ngaySinhDuongLich.getText().toString().trim()
								+ "\n"
								+ "Ngày sinh âm lịch:"
								+ tv_ngaySinhAmLich.getText().toString().trim()
								+ "\n"
